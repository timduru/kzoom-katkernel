...
#define ARM_INT_SKEW_FREQ	1700000
static struct pm_qos_request min_int_qos;

static void exynos5260_set_int_skew_CA15(int new_index)
{
	if (exynos5260_freq_table_CA15[new_index].frequency < ARM_INT_SKEW_FREQ) {
		if (pm_qos_request_active(&min_int_qos))
			pm_qos_update_request(&min_int_qos, 0);
	} else {
		if (pm_qos_request_active(&min_int_qos))
			pm_qos_update_request(&min_int_qos, 266000);
	}
}
#endif
...
#if defined(CONFIG_MACH_M2ALTE) || defined(CONFIG_MACH_M2A3G)
	info->set_int_skew = exynos5260_set_int_skew_CA15;
