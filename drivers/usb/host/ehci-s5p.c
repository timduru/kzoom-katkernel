...
static int s5p_ehci_resume(struct device *dev)
{
	...
	if (pm_qos_request_active(&s5p_ehci_mif_qos))
		pm_qos_update_request(&s5p_ehci_mif_qos, 413000);
	else
		pm_qos_add_request(&s5p_ehci_mif_qos, PM_QOS_BUS_THROUGHPUT, 413000);
...
#endif
	if (pm_qos_request_active(&s5p_ehci_mif_qos))
		pm_qos_update_request(&s5p_ehci_mif_qos, 275000);
	else
		pm_qos_add_request(&s5p_ehci_mif_qos, PM_QOS_BUS_THROUGHPUT, 275000);
